...
class VersionFileHandler(FileSystemEventHandler):
    ...
    def update_firmware(self, version):
        ...
        try:
            ...
            # 构造启动命令: [魔数头][魔数尾][Start_Updata_A命令码][Run_Invalid][固件大小(4字节LE)]
            ...
            ack_received = False
            timeout_counter = 0
            max_timeout = 200  # 200 * 0.1秒 = 20秒超时 (Flash擦除可能需要较长时间)
                ...
                if response:
                    ...
                    for packet in packets:
                        ...
                        if packet['cmd'] == 0x02:  # Cmd_ACK
                            ...
                            ack_received = True
                            break
                        elif packet['cmd'] == 0x01:  # ERROR_Flash_Erasure
            ...
            while total_sent < firmware_size:
                ...
                ack_received = False
                timeout_counter = 0
                max_timeout = 100  # 100 * 0.1秒 = 10秒超时（Flash写入可能需要较长时间）
