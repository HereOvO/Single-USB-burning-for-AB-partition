...
extern uint32_t total_size_to_receive; // 准备接收的bin大小
extern volatile uint16_t received_data_size; // 单次接收字节数
extern volatile uint32_t total_received_data_size; // 总接收字节数
extern volatile uint16_t written_data_size; // 单次写入字节数
extern volatile uint32_t total_written_data_size; // 总写入字节数
extern uint32_t flash_offset; // Flash偏移量
extern volatile uint8_t data_buffer[612];
extern uint8_t last_byte_flag; // 上次接收长度是否为奇数
extern uint8_t last_byte; // 上次剩余的字节
extern volatile uint8_t is_data_buffer_full;
extern volatile uint16_t data_buffer_offset;

extern uint8_t is_start_transmission; // 是否开始传输
extern uint8_t is_transmission_complete; // 传输结束标志
extern uint8_t is_jump_to_application; // 是否跳转到应用程序
extern uint8_t first_reception; // 首次接收标志
extern volatile uint16_t TimerCounter_ms; // 定时器计数（ms）
extern uint32_t expected_crc_value; // 期望的CRC值
extern uint32_t calculated_crc_value; // 计算的CRC值
extern BootloaderPacket_t BootloaderPacket_TX; // 发送数据包
extern BootloaderPacket_t BootloaderPacket_RX; // 接收数据包
...
void ResetAll(void); // 重置相关变量和状态
HAL_StatusTypeDef Download_Flash(uint32_t Size);
HAL_StatusTypeDef FlashErase(uint32_t flash_start_addr, uint32_t Size);

// AB分区管理函数
void LoadPartitionStatus(void);
void SavePartitionStatus(void);
void SwitchToUpdatePartition(void);
void SwitchToPreviousPartition(void);
uint8_t GetCurrentBootPartition(void);
uint8_t GetNextUpdatePartition(void);
void UpdatePartitionStatus(uint8_t status, uint32_t size, uint32_t crc);
