...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  if(BootloaderState.StartMode==AllBootloaderStartModes.Start_Invalid){
    ...
    uint8_t cmd=Buf[2];
    if(cmd==AllCMDs.Cmd_Jump_To_A){
      BootloaderState.StartMode=AllBootloaderStartModes.Start_Dirrectly_Jump_To_A;
    }
    else if(cmd==AllCMDs.Cmd_CopyB_To_A_Jump_To_A){
      BootloaderState.StartMode=AllBootloaderStartModes.Start_CopyB_To_A_Jump_To_A;
